...
(function(){
  ...
  function runTaskCommand(command,payload){
    ...
    if(command==='START_TASK'){
      task=resolveTask(payload,['open']);
      if(typeof startTask!=='function') throw new Error('startTask fehlt.');
      startTask(task.id);
      ...
      task=resolveTask(payload,['running']);
      if(typeof pauseTask!=='function') throw new Error('pauseTask fehlt.');
      pauseTask(task.id);
      if(task.status!=='paused') throw new Error('Aufgabe konnte nicht pausiert werden.');
    } else if(command==='RESUME_TASK'){
      task=resolveTask(payload,['paused']);
      if(typeof resumeTask!=='function') throw new Error('resumeTask fehlt.');
      resumeTask(task.id);
      if(task.status!=='running') throw new Error('Aufgabe konnte nicht fortgesetzt werden.');
    } else if(command==='COMPLETE_TASK'){
      task=resolveTask(payload,['running','paused']);
      if(typeof finishTask!=='function') throw new Error('finishTask fehlt.');
      finishTask(task.id);
  ...
  function runWeightCommand(command,payload){
    if(command==='START_WEIGHT'){
      const kg=Number(String(payload&&payload.weight_kg!=null?payload.weight_kg:'').replace(',','.'));
  ...
  async function processOne(client,row){
    try{
      let result={};
      if(row.command==='ADD_TASK'){
        const task=createLocalTask(row.payload); result={local_task_id:task.id,text:task.text,status:task.status};
      } else if(['START_TASK','PAUSE_TASK','RESUME_TASK','COMPLETE_TASK'].includes(row.command)){
        const task=runTaskCommand(row.command,row.payload); result={local_task_id:task.id,text:task.text,status:task.status,started_at:task.startedAt||null,paused_at:task.pausedAt||null,completed_at:task.completedAt||null};
      } else if(['START_WEIGHT','STOP_WEIGHT'].includes(row.command)){
        result=runWeightCommand(row.command,row.payload||{});
      } else throw new Error('V436: nicht unterstützter Befehl '+row.command+'.');
      ...
      try{await mark(client,row.id,'error',{error:message});}catch(markError){console.warn('V436 command mark error:',markError);}
      console.warn('V436 remote command:',error);
      return false;
    }
  }
  ...
  async function poll(){
    ...
    }catch(error){console.warn('V436 remote command poll:',error);}finally{busy=false;}
  }
  ...
  window.__modRemoteCommandsV436={poll,pollMs:POLL_MS};
})();
