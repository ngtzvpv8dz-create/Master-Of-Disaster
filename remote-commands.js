...
(function(){
  ...
  function runTaskCommand(command,payload){
    let task;
    if(command==='START_TASK'){task=resolveTask(payload,['open']);startTask(task.id);}
    else if(command==='PAUSE_TASK'){task=resolveTask(payload,['running']);pauseTask(task.id);}
    else if(command==='RESUME_TASK'){task=resolveTask(payload,['paused']);resumeTask(task.id);}
    else if(command==='COMPLETE_TASK'){task=resolveTask(payload,['running','paused']);finishTask(task.id);}
  ...
  function runWeightCommand(command,payload){
    ...
    if(command==='START_WEIGHT'){
      const kg=Number(String(payload.weight_kg!=null?payload.weight_kg:'').replace(',','.')),requestedAt=isoOrNull(payload.at);
    ...
    if(command==='ADJUST_WEIGHT_START'){
      ...
      saveWeightAndRender();
      return {weight_kg:Number(weightState.currentPhaseWeightKg||weightState.currentWeightKg)||null,started_at:requestedAt};
    }
    if(command==='STOP_WEIGHT'){
      if(!weightState||!weightState.isWearing)throw new Error('Kein Zusatzgewicht aktiv.');
      const kg=Number(weightState.currentPhaseWeightKg||weightState.currentWeightKg)||null,startedAt=weightState.currentPhaseStartedAt;
      stopWeightPhase();
  ...
  function integrityResult(){
    if(typeof collectDataIntegrityReport!=='function')throw new Error('collectDataIntegrityReport fehlt.');
    const r=collectDataIntegrityReport()||{};
    return {ok:!!r.ok,errors:Array.isArray(r.errors)?r.errors.map(String):[],warnings:Array.isArray(r.warnings)?r.warnings.map(String):[],info:Array.isArray(r.info)?r.info.map(String):[],weight_state:{is_wearing:!!(weightState&&weightState.isWearing),started_at:weightState&&weightState.currentPhaseStartedAt||null,weight_kg:Number(weightState&&((weightState.currentPhaseWeightKg!=null)?weightState.currentPhaseWeightKg:weightState.currentWeightKg))||null},weight_phases_count:Array.isArray(weightPhases)?weightPhases.length:null,open_weight_phases:Array.isArray(weightPhases)?weightPhases.filter(p=>p&&p.startedAt&&!p.endedAt).length:null};
  }

  function taskDetailResult(payload){
    const task=resolveTask(payload,[]);
    return {id:task.id,text:task.text,status:task.status,type:task.type,category:task.category??null,startedAt:task.startedAt??null,pausedAt:task.pausedAt??null,completedAt:task.completedAt??null,abortedAt:task.abortedAt??null,activeDurationMs:task.activeDurationMs??null,actualDurationMs:task.actualDurationMs??null,pauseTotalMs:task.pauseTotalMs??0,activeSegments:Array.isArray(task.activeSegments)?task.activeSegments.map(s=>({...s})):[]};
  }
  ...
  async function processOne(client,row){
    try{
      let result={};
      if(row.command==='REPORT_INTEGRITY')result=integrityResult();
  ...
  async function poll(){
    ...
    try{
      ...
      if(!s)return;
      const {data,error}=await s.client.from('remote_commands').select('id,command,payload,created_at').eq('user_id',s.userId).eq('status','pending').order('created_at',{ascending:true}).limit(10);
      if(error)throw error;
