...
(function(){
  'use strict';
  if(window.__modShoppingV644)return;
  ...
  let state={general:[],food:null,reviews:[],products:[],aliases:[],loading:false,error:'',foodError:''};
    ...
    loadPromise=(async()=>{
      ...
      const [reviewResult,productResult,aliasResult]=await Promise.all([
        supabase.from('shopping_receipt_reviews')
          .select('id,finance_item_id,retailer,receipt_label,normalized_label,status,product_id,notes,created_at,updated_at,resolved_at')
      ...
      try{
        ...
        console.warn('V643 Shopping Food-Bridge:',error);
        state.food=null;
      ...
      console.error('V643 Shopping:',error);
      state.loading=false;
  ...
  function sectionMarkup(id,title,subtitle,rows){
  ...
  function shell(){
    ...
    return '<div class="shopping-hero-v643"><div><span>SHOPPING CONTROL</span><h2>EINKAUFSLISTE</h2><p>Food-Bedarf und normale Einkäufe an einem Ort. Ohne Zettel-Zoo.</p></div>'
      ...
      +sectionMarkup('cart','IM EINKAUFSWAGEN','liegt schon drin',cart)
      +'</div>'
      +reviewMarkup();
  }
  ...
  async function confirmReview(reviewId,productId){
    ...
    if(!exactAlias){
      const aliasResult=await supabase.from('shopping_receipt_aliases').insert({
        ...
        receipt_label:review.receipt_label,
        normalized_label:review.normalized_label,
        product_id:product.id,
        is_default:existingAliases.length===0
      });
      if(aliasResult.error)throw aliasResult.error;
    }
  ...
  const api={
    version:VERSION,open,close,render,reload,
    getState:()=>({
    ...
    centralShopping:true,foodDemandIsDynamic:true,productReviewPlanned:false,productReviewActive:true
  };
