/* V646 · SHOPPING / POLISHED CHECKOUT + PRODUCT REVIEW
...
(function(){
  'use strict';
  if(window.__modShoppingV646)return;
  ...
  function purchasedMarkup(){
    ...
    return '<section class="shopping-purchased-v646"><header><div><span>GEKAUFT</span><small>Der Einkauf ist abgeschlossen. Jetzt nur noch den Bon sauber zuordnen.</small></div><strong>'+checkouts.length+'</strong></header><div class="shopping-purchased-list-v646">'
      +checkouts.map(checkout=>{
        const openCount=(state.reviews||[]).filter(review=>String(review.checkout_id||'')===String(checkout.id)).length;
        const openText=openCount===1?'1 Bonposition offen':openCount+' Bonpositionen offen';
        return '<article class="shopping-purchased-card-v646">'
          +'<div class="shopping-purchased-main-v646"><small>'+esc(when(checkout.completed_at))+'</small><strong>'+esc(checkout.retailer||'Einkauf')+'</strong><span>'+esc(money(checkout.total_amount))+'</span></div>'
          +'<div class="shopping-purchased-status-v646"><b>'+esc(openText)+'</b><span>Gegenprüfung läuft</span></div>'
          +'<button type="button" data-review-jump="'+esc(checkout.id)+'">Zum Bon-Check <b aria-hidden="true">›</b></button>'
          +'</article>';
      }).join('')
      +'</div></section>';
  }
  ...
  function reviewMarkup(){
    ...
    if(!reviews.length){
      return '<section id="shoppingReviewV646" class="shopping-review-v644 is-empty"><header><div><span>EINKAUF GEGENPRÜFEN</span><small>Bonpositionen mit Produktbezug</small></div><strong>0</strong></header><div class="shopping-review-empty-v644">Nichts offen. Der Produktdetektiv hat Pause.</div></section>';
    }
    return '<section id="shoppingReviewV646" class="shopping-review-v644"><header><div><span>BON GEGENPRÜFEN</span><small>Produkt erkennen, bestätigen oder neue Daten nachreichen</small></div><strong>'+reviews.length+'</strong></header><div class="shopping-review-list-v644">'
      +reviews.map(review=>{
  ...
  function bind(root){
    ...
    root.querySelectorAll('[data-review-jump]').forEach(button=>button.addEventListener('click',()=>{
      document.getElementById('shoppingReviewV646')?.scrollIntoView({behavior:'smooth',block:'start'});
    }));
