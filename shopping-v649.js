/* V649 · SHOPPING / TAP FEEDBACK + RECEIPT QUANTITIES
...
(function(){
  'use strict';
  if(window.__modShoppingV649)return;
    ...
    return '<section id="shoppingReviewV647" class="shopping-review-v644"><header><div><span>BON GEGENPRÜFEN</span><small>Produkt erkennen, bestätigen oder neue Daten nachreichen</small></div><strong>'+reviews.length+'</strong></header><div class="shopping-review-list-v644">'
      +reviews.map(review=>{
        ...
        return '<article class="shopping-review-item-v644 '+(pending?'is-pending-data':'')+'">'
          +'<div class="shopping-review-copy-v644"><small>'+esc(review.retailer||'Händler')+(receipt.detail?' · '+esc(receipt.detail):'')+'</small><strong>'+esc(review.receipt_label||'Bonposition')+(receipt.qty?'<span class="shopping-review-qty-v649">'+esc(receipt.qty)+'</span>':'')+'</strong>'
          +(pending
