...
(function(){
  ...
  function modeFor(name=tab()){
    const modes=readJson(MODE_KEY,{});
    return modes[name]===GROUPED?GROUPED:ORDER;
  }
  function setMode(name,mode){
    if(!GROUP_TABS.has(name))return false;
    const modes=readJson(MODE_KEY,{});
    modes[name]=mode===GROUPED?GROUPED:ORDER;
    writeJson(MODE_KEY,modes);
  ...
  function injectStyle(){
    ...
    style.id='categoryGroupDragV473Style';
    style.textContent=`
      ...
      .task-category-items-v473{padding:7px 7px 0}
      .task-category-items-v473.collapsed{display:none}
      ...
      body .today-task-active.drag-insert-before-v473::before{top:-6px}
      body .today-task-active.drag-insert-after-v473::after{bottom:-6px}
  ...
  function rowForCard(card){
    ...
    if(Number.isFinite(id)&&Array.isArray(window.tasks||tasks)){
      const direct=(window.tasks||tasks).find(row=>Number(row?.id)===id);
  ...
  function categoryForCard(card){
    ...
    const value=clean(row?.category);
    return value||'OHNE KATEGORIE';
  }

  function visibleCards(root){
    const selector='.task, .archive-task';
    const all=[...new Set([...root.querySelectorAll(selector)])];
  ...
  function sectionName(root,index){
    if(root.id==='viewContainer')return `Hauptliste ${index+1}`;
    return clean(root.querySelector('.section-title')?.textContent)||`Bereich ${index+1}`;
  }

  function createGroup(tabName,sectionLabel,category,cards){
    const key=collapseKey(tabName,sectionLabel,category);
    const collapsed=isCollapsed(key);
    const wrap=document.createElement('div');
    wrap.className='task-category-group-v473';
    wrap.dataset.v473Category=category;
    const head=document.createElement('button');
    head.type='button';
    head.className=`task-category-head-v473${collapsed?' collapsed':''}`;
    head.setAttribute('aria-expanded',String(!collapsed));
    head.title=`Kategorie ${category} ${collapsed?'aufklappen':'zuklappen'}`;
    ...
    const chev=document.createElement('span');chev.className='task-category-chevron-v473';chev.textContent='▾';
    head.append(name,count,chev);
    const body=document.createElement('div');body.className=`task-category-items-v473${collapsed?' collapsed':''}`;
  ...
  function applyGrouping(){
    const tabName=tab();
    if(!GROUP_TABS.has(tabName)||modeFor(tabName)!==GROUPED)return false;
    const container=document.getElementById('viewContainer');
    if(!container||container.querySelector('.task-category-group-v473'))return false;
    container.classList.add('category-view-grouped-v473');
    const roots=sectionRoots(container);
    roots.forEach((root,index)=>{
      const cards=visibleCards(root);
      if(!cards.length)return;
      const marker=document.createElement('span');marker.hidden=true;marker.dataset.v473GroupMarker='1';
      cards[0].parentNode.insertBefore(marker,cards[0]);
      ...
      const label=sectionName(root,index);
      buckets.forEach((groupCards,category)=>fragment.appendChild(createGroup(tabName,label,category,groupCards)));
    ...
    if(tabName==='today'){
      container.querySelectorAll('.drag-handle').forEach(handle=>{
        ...
        handle.title='Zum Sortieren auf REIHENFOLGE wechseln';
      });
    }
    return true;
  }
  ...
  function injectViewToggle(){
    const tabName=tab();
    if(!GROUP_TABS.has(tabName))return false;
    const container=document.getElementById('viewContainer');
    if(!container||document.getElementById('categoryViewToggleV473'))return false;
    const current=modeFor(tabName);
    const bar=document.createElement('div');
    bar.id='categoryViewToggleV473';
    bar.className=`category-view-toggle-v473${current===GROUPED?' grouped':''}`;
    ...
    const grouped=document.createElement('button');grouped.type='button';grouped.className=`category-view-button-v473${current===GROUPED?' active':''}`;grouped.textContent='▦ GRUPPIERT';grouped.dataset.v473Mode=GROUPED;
    bar.append(label,order,grouped);
    ...
    const categoryToolbar=document.getElementById('categoryToolbarV412');
    if(categoryToolbar)categoryToolbar.insertAdjacentElement('afterend',bar);else container.prepend(bar);
    return true;
  }

  function todayOrderRows(){
    if(!Array.isArray(window.tasks||tasks)||typeof getBerlinDateKey!=='function')return [];
    const today=getBerlinDateKey();
    return [...(window.tasks||tasks)]
      .filter(task=>task?.todayDate===today&&!['completed','aborted'].includes(String(task?.status||'')))
      .sort((a,b)=>(Number(a.todayOrder)||0)-(Number(b.todayOrder)||0));
  }

  function getDropPreview(draggedId,targetId){
    const rows=todayOrderRows();
    ...
    return {position:to+1,total:rows.length,side:from<to?'after':from>to?'before':'self',from:from+1,to:to+1};
  }

  function clearDragFeedback(){
    document.querySelectorAll('.drag-insert-before-v473,.drag-insert-after-v473').forEach(card=>card.classList.remove('drag-insert-before-v473','drag-insert-after-v473'));
    document.querySelectorAll('.drag-position-badge-v473').forEach(badge=>badge.remove());
  }

  function ensurePositionBadge(card,preview){
    if(!card||!preview)return null;
    let badge=card.querySelector('.drag-position-badge-v473');
    if(!badge){
      badge=document.createElement('span');badge.className='drag-position-badge-v473';
      const handle=card.querySelector('.drag-handle');
      if(handle)handle.insertAdjacentElement('beforebegin',badge);else card.appendChild(badge);
    }
    badge.textContent=`POSITION ${preview.position} / ${preview.total}`;
    return badge;
  }

  function applyDragFeedback(draggedId,targetId){
    if(tab()!=='today'||modeFor('today')===GROUPED)return null;
    const preview=getDropPreview(draggedId,targetId);if(!preview)return null;
    ...
    const dragged=container.querySelector(`.today-task-active[data-id="${CSS.escape(String(draggedId))}"]`);
    const target=container.querySelector(`.today-task-active[data-id="${CSS.escape(String(targetId))}"]`);
    ensurePositionBadge(dragged,preview);
    if(target&&preview.side==='before')target.classList.add('drag-insert-before-v473');
    if(target&&preview.side==='after')target.classList.add('drag-insert-after-v473');
    return preview;
  }

  function nearestTodayTarget(event){
    const container=document.getElementById('viewContainer');if(!container)return null;
    const direct=document.elementFromPoint?.(event.clientX,event.clientY)?.closest?.('.today-task-active');
    if(direct&&container.contains(direct))return direct;
    const cards=[...container.querySelectorAll('.today-task-active')];
    let best=null,bestDistance=Infinity;
    cards.forEach(card=>{const rect=card.getBoundingClientRect();const mid=rect.top+rect.height/2;const d=Math.abs(event.clientY-mid);if(d<bestDistance){bestDistance=d;best=card;}});
    return best;
  }

  function onPointerDown(event){
    if(tab()!=='today'||modeFor('today')===GROUPED)return;
    const handle=event.target?.closest?.('.drag-handle');if(!handle)return;
    setTimeout(()=>{
      const card=handle.closest('.today-task-active');if(!card)return;
      const id=Number(card.dataset.id);const preview=getDropPreview(id,id);if(preview)ensurePositionBadge(card,preview);
    },0);
  }

  function onPointerMove(event){
    if(tab()!=='today'||modeFor('today')===GROUPED)return;
    const dragged=document.querySelector('#viewContainer .today-task-active.dragging');if(!dragged)return;
    const target=nearestTodayTarget(event);if(!target)return;
    applyDragFeedback(Number(dragged.dataset.id),Number(target.dataset.id));
  }
  ...
  function afterRender(){
    ...
    const tabName=tab();
    const container=document.getElementById('viewContainer');
    if(container)container.classList.remove('category-view-grouped-v473');
    if(!GROUP_TABS.has(tabName))return;
    injectViewToggle();
    applyGrouping();
  }
  ...
  if(previousRender){
    render=function(){const result=previousRender.apply(this,arguments);afterRender();return result;};
  }
  ...
  window.addEventListener('load',()=>setTimeout(afterRender,220));
  setTimeout(afterRender,0);

  window.__modCategoryGroupDragV473={
    ...
    applyDragFeedback,
    clearDragFeedback,
    groupingIsViewOnly:true,
    todayDragGroupedDisabled:true,
    livePosition:true,
    insertionLine:true
  };
})();
