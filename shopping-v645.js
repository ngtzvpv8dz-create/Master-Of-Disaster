/* V645 · SHOPPING / CHECKOUT + PRODUCT REVIEW
...
(function(){
  'use strict';
  if(window.__modShoppingV645)return;
  ...
  function setSurface(active){
    ...
    if(active){
      ...
      if(meta)meta.setAttribute('content','#17130d');
    }else{
  ...
  async function loadData({refreshFood=true}={}){
    ...
    loadPromise=(async()=>{
      ...
      try{
        ...
        console.warn('V645 Shopping Food-Bridge:',error);
        state.food=null;
      ...
      console.error('V645 Shopping:',error);
      state.loading=false;
  ...
  function sectionMarkup(id,title,subtitle,rows,action=''){
    return '<section class="shopping-section-v643 is-'+id+'">'
      +'<header><div><span>'+esc(title)+'</span><small>'+esc(subtitle)+'</small></div><div class="shopping-section-tools-v645">'+action+'<strong>'+rows.length+'</strong></div></header>'
  ...
  function purchasedMarkup(){
    ...
    return '<section class="shopping-purchased-v645"><header><div><span>GEKAUFT · GEGENPRÜFUNG OFFEN</span><small>Diese Positionen sind aus dem aktiven Einkaufszettel raus und warten auf den Bon-Abgleich.</small></div><strong>'+items.length+'</strong></header><div>'
      +items.map(item=>'<article><span>'+esc(item.label)+'</span><b>'+esc(item.quantity!==null&&item.quantity!==undefined?fmtQty(item.quantity,item.unit||''):'gekauft')+'</b></article>').join('')
      +'</div></section>';
  }
  ...
  function reviewMarkup(){
    ...
    if(!reviews.length){
      return '<section class="shopping-review-v644 is-empty"><header><div><span>EINKAUF GEGENPRÜFEN</span><small>Bonpositionen mit Produktbezug</small></div><strong>0</strong></header><div class="shopping-review-empty-v644">Nichts offen. Der Produktdetektiv hat Pause.</div></section>';
    }
    return '<section class="shopping-review-v644"><header><div><span>EINKAUF GEGENPRÜFEN</span><small>Bekanntes Produkt bestätigen oder neues markieren</small></div><strong>'+reviews.length+'</strong></header><div class="shopping-review-list-v644">'
      +reviews.map(review=>{
        ...
        return '<article class="shopping-review-item-v644 '+(pending?'is-pending-data':'')+'">'
          ...
          +(candidate&&!pending?'<button type="button" data-review-confirm="'+esc(review.id)+'">✓ Das ist es</button>':'')
          +'<button type="button" data-review-choose="'+esc(review.id)+'">Produkt wählen</button>'
          +'<button type="button" data-review-pending="'+esc(review.id)+'">Anderes · Daten kommen noch</button>'
          +'<button type="button" class="is-quiet" data-review-ignore="'+esc(review.id)+'">Ohne Produktstamm</button>'
          +'</div></article>';
      }).join('')
      +'</div></section>';
  }
  ...
  function shell(){
    ...
    return '<div class="shopping-hero-v643"><div><span>SHOPPING CONTROL</span><h2>EINKAUFSLISTE</h2><p>Food-Bedarf und normale Einkäufe an einem Ort. Ohne Zettel-Zoo.</p></div>'
      ...
      +sectionMarkup('cart','IM EINKAUFSWAGEN','liegt schon drin',cart,cart.length?'<button type="button" class="shopping-checkout-button-v645" data-shopping-checkout>EINKAUF ABSCHLIESSEN</button>':'')
      +'</div>'
      +purchasedMarkup()
      +reviewMarkup();
  }
