...
(function(){
  'use strict';
  if(window.__modAreaRuntimeV608)return;
  ...
  function emitProgress(detail){
    emit('mod:bootstrap-v608-progress',detail);
  ...
  function heavyDelay(context,src){
    ...
    if(context==='log'||context==='history')return 320;
    if(context==='backup'||context==='restore')return 220;
    return 28;
  }
  ...
  function loadScript(src){
    ...
    const promise=new Promise((resolve,reject)=>{
      ...
      script.src=src;
      script.async=false;
      script.dataset.modAreaRuntimeV608='true';
      script.onload=()=>{
        executed.add(key);
  ...
  function beginAreaProgress(id,total){
    loadingArea=id;
    lastFailedArea=null;
    document.documentElement.classList.add('mod-area-loading-v608');
  ...
  function finishAreaProgress(id,total){
    ...
    loadingArea=null;
    document.documentElement.classList.remove('mod-area-loading-v608');
    delete document.documentElement.dataset.modAreaLoadingV608;
  }
  ...
  async function warmFallback(entries){
    ...
    for(const entry of entries){
      try{
        ...
        console.warn('V608 optional warmup:',entry.url,error);
      }
      done++;
  ...
  async function preloadAll(){
    ...
    running=(async()=>{
      try{
        ...
        if(gateState?.mode==='test-bypass'){
          ...
          if('serviceWorker' in navigator&&navigator.serviceWorker.controller){
            warmed=await warmViaWorker(entries).catch(error=>{
              console.warn('V608 Worker-Warmup fallback:',error);
              return false;
            });
          }
        ...
        emit('mod:bootstrap-v608-ready',detail);
        ...
        console.error('V608 Bootstrap:',error);
  ...
  async function loadSection(section){
    ...
    await loadListSequential(section,list,{showProgress:false});
  ...
  async function openArea(id){
    ...
    if(id==='todo'){
      ...
      result=window.__modAppHubV515?.open?.('todo',{source:'v608-ready'});
    }else if(id==='sport'){
      result=window.__modAppHubV515?.open?.('sport',{source:'v608-ready'});
    }else if(id==='food'){
  ...
  document.addEventListener('click',event=>{
    ...
    if(launcher){
      ...
      openArea(id).catch(error=>console.error('V608 Bereich:',id,error));
      return;
    }
    ...
    if(historyButton&&!window.__modRecoveryHistoryV498){
      ...
      loadSection('history')
        ...
        .catch(error=>console.error('V608 History:',error));
      return;
    }
    ...
    if(section==='log'){
      ...
      }).catch(error=>console.error('V608 Log:',error))
    ...
    }).catch(error=>console.error('V608 Backstage:',section,error))
      .finally(()=>sectionButton.removeAttribute('aria-busy'));
  },true);
