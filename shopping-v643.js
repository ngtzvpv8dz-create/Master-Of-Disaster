/* V643 · SHOPPING / CENTRAL SHOPPING LIST
...
(function(){
  'use strict';
  if(window.__modShoppingV643)return;
  ...
  let state={general:[],food:null,loading:false,error:'',foodError:''};
  ...
  function shell(){
    ...
    const foodCount=rows.filter(row=>row.source.startsWith('food')).length;
    ...
    return '<div class="shopping-hero-v643"><div><span>SHOPPING CONTROL</span><h2>EINKAUFSLISTE</h2><p>Food-Bedarf und normale Einkäufe an einem Ort. Ohne Zettel-Zoo.</p></div>'
      ...
      +'<div class="shopping-summary-v643"><div><strong>'+now.length+'</strong><span>Einkaufen</span></div><div><strong>'+later.length+'</strong><span>Später</span></div><div><strong>'+cart.length+'</strong><span>Im Wagen</span></div><div><strong>'+foodCount+' / '+generalCount+'</strong><span>Food / sonstige</span></div></div>'
      +warning
      ...
      +'</div>'
      +'<section class="shopping-review-teaser-v643"><div><span>NÄCHSTER BAUSTEIN</span><strong>Einkauf gegenprüfen</strong><small>Kassenbon → bekanntes Produkt zuordnen oder „anderes Produkt / Daten kommen noch“.</small></div><b>🧾</b></section>';
  }
  ...
  const api={
    ...
    getState:()=>({general:state.general.map(item=>({...item})),food:state.food?structuredClone(state.food):null}),
    centralShopping:true,foodDemandIsDynamic:true,productReviewPlanned:true
  };
