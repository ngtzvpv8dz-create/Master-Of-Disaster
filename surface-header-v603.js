...
(function(){
  'use strict';
  if(window.__modSurfaceHeaderV603)return;
  ...
  function resetFit(eyebrow){
    eyebrow.style.setProperty('--mod-tagline-size-v603','11px');
    eyebrow.style.setProperty('--mod-tagline-letter-v603','1.35px');
    eyebrow.style.setProperty('--mod-tagline-scale-v603','1');
  }
  ...
  function fitOneLine(eyebrow){
    ...
    fitRaf=requestAnimationFrame(()=>{
      ...
      while(eyebrow.scrollWidth>available+.5&&size>8.15&&guard<18){
        ...
        eyebrow.style.setProperty('--mod-tagline-size-v603',size.toFixed(2)+'px');
        eyebrow.style.setProperty('--mod-tagline-letter-v603',spacing.toFixed(2)+'px');
        guard++;
      }
      const width=Math.max(1,eyebrow.scrollWidth);
      const scale=width>available?Math.max(.80,Math.min(1,available/width)):1;
      eyebrow.style.setProperty('--mod-tagline-scale-v603',scale.toFixed(4));
      eyebrow.dataset.modTaglineFitV603=scale<.999?'condensed':(size<10.99||spacing<1.34?'compact':'native');
  ...
  function apply(){
    ...
    eyebrow.classList.remove('mod-surface-tagline-v601','mod-surface-tagline-v602');
    eyebrow.classList.add('mod-surface-tagline-v603');
    eyebrow.dataset.modTaglineSurfaceV603=surface;
    eyebrow.textContent=entry.text;
    eyebrow.style.setProperty('--mod-tagline-color-v603',entry.color);
    fitOneLine(eyebrow);
    return true;
  }
  ...
  window.__modSurfaceHeaderV603={version:VERSION,taglines:{...TAGLINES},apply,currentSurface,fitOneLine};
